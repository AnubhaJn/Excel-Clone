...
$(".menu-selector").change(function (e) {   // when drop down option is changed
    let value = $(this).val();   // other way is- this.value 
    ...
    if (!isNaN(value)) {   // checks if the value is number or not
        value = parseInt(value);   //changing valur in integer
    }
...
function setFontStyle(ele, property, key, value) {
    if ($(ele).hasClass("selected")) {
        ...
        // $(".input-cell.selected").each(function (index, data) {
        //     let [rowId, colId] = findRowCol(data);
        //     cellData[selectedSheet][rowId - 1][colId - 1][property] = false; // here we have minus 1 because our rows and colum started from 1 but array indexing from 0
        // });
        ...
        // $(".input-cell.selected").each(function (index, data) {
        //     let [rowId, colId] = findRowCol(data);
        //     cellData[selectedSheet][rowId - 1][colId - 1][property] = true; // here we have minus 1 because our rows and colum started from 1 but array indexing from 0
        // })
...
function updateCellData(property, value) {
    ...
    if (value != defaultProperties[property]) { // check we should not have clicked the default property
        $(".input-cell.selected").each(function (index, data) {
            let [rowId, colId] = findRowCol(data);
            if (cellData[selectedSheet][rowId - 1] == undefined) { //if till now there is no change in the row selected then there will be no row yet 
                cellData[selectedSheet][rowId - 1] = {};  //make new row so that we can store the changes 
        ...
        $(".input-cell.selected").each(function (index, data) {
            ...
            if (cellData[selectedSheet][rowId - 1] && cellData[selectedSheet][rowId - 1][colId - 1]) {
                ...
                if (JSON.stringify(cellData[selectedSheet][rowId - 1][colId - 1]) == JSON.stringify(defaultProperties)) {
                    ...
                    if (Object.keys(cellData[selectSheet][rowId - 1]).length == 0) {
...
function addSheetTabEventListeners() {
    $(".sheet-tab.selected").bind("contextmenu", function (e) {
        ...
        $(".sheet-options-modal").css({ "bottom": 0.04 * $(window).height(), "left": e.pageX }); // page x gives coordinate from left and page y from top //$(window).height() gives he height of window
        $(".sheet-rename").click(function (e) {
            ...
            $(".cancel-button,.cross-icon").click(function (e) {
...
function renameSheet() {
    ...
    if (newSheetName && !Object.keys(cellData).includes(newSheetName)) {
        ...
        $(".error").remove();
        if (Object.keys(cellData).includes(newSheetName)) {
            $(".sheet-modal-input-container").append(`<div class="error"> Sheet already exists! </div>`);
        }
        if (!newSheetName) {
            $(".sheet-modal-input-container").append(`<div class="error"> Sheet Name is not Valid </div>`);
        }
    }
}
    ...
    $(".new").click(function (e) {
        if (saved) {
            ...
            <div class="sheet-delete-modal">
                ...
                <div class="sheet-modal-detail-container">
                    <div class="material-icons saved
...
function newFile() {  //we load a new sheet by default book that we star from
    emptySheet();
    $(".sheet-tab").remove();
    $(".sheet-tab-conainer").append(`<div class="sheet-tab selected">Sheet1</div>`);
    cellData = { "Sheet1": {} };
